...
if(isset($_POST['MAKE']))
{
    ...
    $m3 = $gross/3;
    $m6 = $gross/6;
    $result = mysqli_query($con,"SELECT * FROM gross_rate WHERE g_id='".$n_rate[0]."' ");
    while($row = mysqli_fetch_array($result))
    {
      $g_rate = $row['g_rate'];
    }
array_push($response,array("MAKE"=>$make,"VAR"=>$var,"MODEL"=>$model,"YM"=>$year,"COLOR"=>$color,"rbi"=>"₱".$rbi1.".00","rpd"=>"₱".$rpd1.".00","TRANS"=>$trans,"FMV"=>"₱".number_format($_POST['fmv'],2),"AOG"=>"₱".number_format($aog1,2),"GROSS"=>$n_rate[1],"M3"=>"₱".number_format($m3,2),"M6"=>"₱".number_format($m6,2),"ref"=>$unique,"name"=>$name,"usage"=>$usage,"RATE"=>$g_rate));
...
else if(isset($_POST['BRAND']))
{
    ...
    {
    echo "INSERT INTO user_quotes ( `tracking_no`, `aog`, `gross`, `fmv_id`, `rbi`, `rpd`, `client_name`, `markup`, `date_created`, `a_id`) VALUES ('".$ref."','".$aog."','".$gross."','".$fmv_id."','".$rbi."','".$rpd."','".$name."','".$mark."','".$date."','".$id."'";   
    }  
}
?>
